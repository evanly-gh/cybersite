...
/**
 * Merged single-tube track builder.
 *
 * Builds ONE TubeGeometry per track feature (girder, deck, running lights, cable
 * tray) over the FULL ~2000m loop plus ONE InstancedMesh for ribs — totalling
 * ~6 track meshes + ~4 pylon draws = a fixed ~10 draw calls regardless of
 * viewpoint. This is far cheaper than a zone-split approach for this loop, because
 * the metro threads EVERY visible zone so from any street camera you see multiple
 * zones; splitting multiplies mesh count without achieving frustum culling.
 */
